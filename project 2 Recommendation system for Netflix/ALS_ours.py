...
def get_ALS_predictions(ratings, train, test, lambda_user, lambda_item, n_features_array, ind = 0, kwargs = None):
    ...
    """
    n_models = n_features_array.size
    ...
    for idx, n_features in enumerate([n_features_array]):
        ...
        np.save('dump/%s_full_ALS_K=%s.npy'%(idx,idx+1),predicted_labels.reshape(-1))
        np.save('dump/%s_test_ALS_k=%s.npy'%(idx,idx+1),predicted_labels[test.nonzero()])
